...
void writeMessageToPNG(const std::string& filename, const std::string& message, uint32_t width, uint32_t height, uint16_t bitsPerPixel) {
    ...
    for (size_t i = dataOffset; i < imageData.size() && bitIndex < binaryMessage.length(); ++i) {
        // Iterate through the bits of the current byte.
        for (int j = 7; j >= 0 && bitIndex < binaryMessage.length(); --j) {
            // Use a conditional expression for conciseness.
            imageData[i] = (binaryMessage[bitIndex] == '1')
                ? (imageData[i] | (1 << j))     // Set bit j to 1
                : (imageData[i] & ~(1 << j));  // Set bit j to 0
            ++bitIndex;
        }
    }
